...
- **Svelte-scoping rule — swapping a native element to `<motion.div>`:** doing so moves that DOM node's rendering into a child component (`@humanspeak/svelte-motion`), so Svelte's own scoping hash is **not** applied to it anymore. Any rule in the parent's `<style>` block that targets that element **as the rightmost/target compound selector** (e.g. a standalone `.sec-head { ... }` rule) will silently stop matching unless rewritten as `:global(.sec-head) { ... }`. **Correction (discovered during Task 14's review, retroactively fixed in commit `b18b3f3`):** the original text of this rule claimed that descendant selectors rooted at the swapped class (e.g. `.sec-head h2`) needed no change, reasoning "Svelte only ever hash-scopes the rightmost compound selector." That reasoning was wrong and caused a real, confirmed production regression across Features/Modes/Download/Footer — Svelte's dead-CSS-elimination pass strips the *entire* descendant rule from the compiled output whenever the rule's ancestor class lives on a component instead of a native element, regardless of which part of the selector is rightmost. The correct rule: **every** selector rooted at the swapped class — standalone or descendant — must be wrapped in `:global(...)` (e.g. `:global(.sec-head h2)`, not just `:global(.sec-head)`), *unless* the descendant rule is actually keyed off a different, never-swapped native ancestor class in the same file (as is the case for Hero's `.hero h1`/`.hero .sub`, keyed off `.hero` rather than the swapped `.hero-inner`, and CloserLook's `.closer h2`/`.closer-points`, keyed off `.closer` rather than the swapped `.closer-media` — both correctly required no change). When verifying this pattern, check the compiled CSS output directly (`npm run build`, then grep `dist/_app/immutable/assets/*.css`) rather than relying on `npm run check` alone, since a missing declaration is easy to miss among dozens of legitimate pre-existing warnings. When an element has another action or binding worth preserving (e.g. Hero's `use:tilt` + `bind:this`), **wrap** it in an unstyled outer `<motion.div>` instead of swapping its tag — the inner element and all its existing scoped CSS stay untouched.
